...
class test_ConfigLoader(unittest.TestCase):
  def setUp(self):
    ...
    self.empty_config = ConfigLoader('%s/empty.ini' % self.root_path)
    self.full_config = ConfigLoader('%s/full.ini' % self.root_path)
    self.relative_config = ConfigLoader('%s/relative.ini' % self.root_path)
