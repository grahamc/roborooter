...
class test_ConfigLoader(unittest.TestCase):
  def setUp(self):
    ...
    self.empty_config = ConfigLoader('%s/empty.ini' % self.root_path).get_config()
    self.full_config = ConfigLoader('%s/full.ini' % self.root_path).get_config()
    self.relative_config = ConfigLoader('%s/relative.ini' % self.root_path).get_config()
  ...
  def test_minimum_version_above_default_version(self):
    with self.assertRaises(ValueError):
      ConfigLoader('%s/default_version_below_minimum.ini' % self.root_path)
